...
증분 fetch (워터마크):
  ...
  - **재시도 대기**: 직전 싱크에서 임포트가 실패해
    ``confluence_sync_fetch_retries`` 에 기록된 페이지. 변경 후보 조회와
    무관하게 항상 fetch 한다.

  워터마크는 Phase 1 임포트 실패 페이지를 재시도 대기 목록에 기록한 뒤
  싱크 시작 시각으로 전진한다 — 실패 페이지는 다음 싱크에서 강제 fetch
  되므로 워터마크가 그 변경 시각을 지나쳐도 유실이 없다. 재시도 목록
  기록/조회가 실패한 경우에만 워터마크를 유지한다. (종전에는 오류가 1건
  이라도 있으면 전진하지 않았는데, 영구 실패 페이지 소수가 워터마크를
  멈춰 매 사이클 전량 fetch 를 유발했다.) 워터마크가 없거나(첫 싱크) 변경
  후보 조회가 실패하면 전체 fetch 로 폴백한다. fetch 여부와 무관하게 최종
  재인덱싱 판정은 기존처럼 content hash 가 담당한다.
...
async def _import_incremental_and_prune(
    ...
    *,
    ...
    워터마크는 Phase 1 임포트 실패 페이지를 재시도 대기 목록에 기록한 뒤
    ``sync_started_at`` 으로 전진한다. 실패 페이지는 다음 싱크에서 변경
    후보 조회와 무관하게 강제 fetch 되므로, 워터마크가 전진해 변경 조회
    범위에서 벗어나도 "기존 문서의 변경" 이 유실되지 않는다. 재시도 목록
    기록 자체가 실패한 경우에만 워터마크를 유지한다 (안전 폴백) — 소수
    페이지의 반복 실패가 워터마크를 영구히 멈춰 매 사이클 전량 fetch 를
    유발하던 종전 all-or-nothing 조건의 완화다.
    """
    ...
    try:
        retry_ids = await meta_store.list_fetch_retry_page_ids(target_id)
    except Exception:  # noqa: BLE001
        # 재시도 목록을 모르면 증분 선정에서 이전 실패분이 빠질 수 있다 —
        # 전체 fetch 로 폴백하고, 아래에서 워터마크도 유지한다.
        logger.warning(
            "fetch 재시도 목록 조회 실패 — 전체 fetch 로 폴백 target_id=%s",
    ...
    # 실패 페이지를 재시도 대기 목록에 기록한 뒤 워터마크를 전진시킨다.
    # 목록 교체(replace) 의미론: 이전 대기분은 이번 실행에서 강제 fetch
    # 됐으므로 성공했으면 빠지고 또 실패했으면 result.errors 로 다시 들어온다.
    # 단, 열거가 잘려 이번에 시도조차 못 한 대기분은 이월한다 — 지우면 그
    # 페이지의 변경이 유실된다 (워터마크는 이미 그 변경 시각을 지나쳤으므로).
    if retry_ids is None:
        # 재시도 목록 조회가 실패한 상태에서 전진하면 이전 실패분의 변경이
        # 유실될 수 있다 — 이번 사이클은 워터마크를 유지한다.
        logger.info(
            "fetch 재시도 목록 조회 실패 — 워터마크 유지 target_id=%s "
            "(다음 싱크에서 재시도)", target_id,
        )
        return

    failed_entries = [
        {"page_id": str(e["page_id"]), "error": str(e.get("error", ""))}
    ...
    if not enumeration_complete:
        ...
        failed_entries.extend(
            {"page_id": pid, "error": "carried over (이번 열거에 미포함)"}
            for pid in sorted(retry_ids)
            if pid not in attempted
        )

    try:
        await meta_store.replace_fetch_retries(target_id, failed_entries)
    except Exception:  # noqa: BLE001
        logger.warning(
            "fetch 재시도 목록 저장 실패 — 워터마크 유지 target_id=%s "
            "(다음 싱크에서 재조회)", target_id, exc_info=True,
        )
        return

    if result.errors:
        logger.info(
            "임포트 오류 %d건 — 재시도 대기 목록에 기록하고 워터마크 전진 "
            "target_id=%s (다음 싱크에서 강제 fetch)",
