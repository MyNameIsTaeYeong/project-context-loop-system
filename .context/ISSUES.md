...
- Phase 9.2
- **진행 상태**: scope_analyzer.py 구현 완료 (2-pass + 레이어 감지). 실제 레포에서 상품 식별 정확도 미달 — 추가 튜닝 필요.

### I-027: scope_analyzer 상품 식별 정확도 개선
- `_detect_layered_products()`의 레이어 감지 + 파일명 기반 식별이 실제 레포에서 기대와 다른 결과를 반환하는 문제
- 현재 구현: 디렉토리 기반 우선 → 파일명 기반 폴백 → LLM Pass 1 폴백
- 가능한 개선 방향:
  - 실제 레포 구조를 수집하여 패턴 분석 후 감지 로직 보강
  - 감지 결과를 사람이 확인/수정하는 대시보드 UI 추가 (I-026과 연계)
  - 혼합 구조(일부 디렉토리, 일부 파일명) 대응 로직 강화
- Phase 9.4+ 관련
