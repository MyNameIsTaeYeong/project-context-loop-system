...
- Phase 9.2
- **진행 상태**: scope_analyzer.py 구현 완료 (2-pass + 레이어 감지). I-027 정확도 개선 완료 (혼합 구조, 오탐 방지, 복수 그룹 탐색). 대시보드 UI 미구현.

### I-027: scope_analyzer 상품 식별 정확도 개선
- `_detect_layered_products()`의 레이어 감지 + 파일명 기반 식별이 실제 레포에서 기대와 다른 결과를 반환하는 문제
- **개선 완료 (3건)**:
  1. 혼합 구조 지원: 디렉토리 기반과 파일 기반 감지를 결합(합집합)하여 혼재 구조에서 상품 누락 방지
  2. 파일명 기반 오탐 방지: 레이어명이 파일명에 포함되지 않은 파일(`utils.py`, `base.py` 등)을 상품으로 오인하지 않도록 수정
  3. 복수 레이어 그룹 탐색: BFS에서 첫 번째 레이어 그룹만 찾던 것을 모노레포 내 모든 레이어 그룹을 탐색하도록 변경
- 추가 개선 방향 (남은 작업):
  - 감지 결과를 사람이 확인/수정하는 대시보드 UI 추가 (I-026과 연계)
  - 실제 레포 구조를 수집하여 추가 패턴 분석 후 감지 로직 보강
- Phase 9.4+ 관련
