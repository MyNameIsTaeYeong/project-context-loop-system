...
- **Step**: 7.5 쿼리 확장 HyDE 완료
- **상태**: Phase 7.1~7.5 구현 완료. 7.5: HyDE (Hypothetical Document Embedding) 기반 쿼리 확장.
...
- 일시: 2026-03-31
- 내용: Phase 7.5 구현 완료.
  - Phase 7.5 (D-022): HyDE 기반 쿼리 확장. LLM으로 가상 답변 문서 생성 → 원본 쿼리 임베딩과 평균하여 의미적으로 풍부한 검색 벡터 생성. `processor/query_expander.py` 신규. `context_assembler.py`에 hyde_enabled 파라미터 추가. 11개 신규 테스트. 전체 240개 통과.
