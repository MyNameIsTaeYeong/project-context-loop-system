...
- **미해결 후속**: REST `SyncEngine`(engine.py) 데드 코드 정리 여부 결정, 9.9 체크박스 정합 확인(`git_repository.py::get_changed_files` 의 diff 기반 증분 + 변경 없음 조기 종료가 이미 구현돼 있어 자동 주기 싱크의 무변경 사이클은 저렴 — 체크박스만 미갱신으로 보임).
