...
@pytest.mark.asyncio
async def test_execute_search_empty_plan_requires_targets_by_default(
    graph_store: GraphStore, meta_store: MetadataStore,
) -> None:
    """target/step 이 모두 빈 계획은 기본(require_targets=True)에서 즉시 None."""
    doc_id = await _create_doc(meta_store)
    await graph_store.save_graph_data(doc_id, GraphData(
    ...
    plan = GraphSearchPlan(should_search=True)
    result = await execute_graph_search(
        plan, graph_store, query_embedding=[1.0, 0.0],
    )
    assert result is None
...
@pytest.mark.asyncio
async def test_execute_search_no_planner_seeds_from_query_embedding_only(
    graph_store: GraphStore, meta_store: MetadataStore,
) -> None:
    """ablation no-planner: 빈 계획 + require_targets=False 면 쿼리 임베딩
    시딩(always-on 보강 / 최후 폴백)만으로 결과를 만든다 — LLM 호출 0회
    베이스라인의 실행 경로."""
    doc_id = await _create_doc(meta_store)
    await graph_store.save_graph_data(doc_id, GraphData(
        entities=[
...
@pytest.mark.asyncio
async def test_execute_search_no_boost_skips_query_augmentation(
    graph_store: GraphStore, meta_store: MetadataStore,
) -> None:
    """ablation no-boost: enable_query_boost=False 면 always-on 쿼리 임베딩
    시드 보강이 생략된다 — 보강 층의 단독 기여를 분리 측정 가능."""
    doc_id = await _create_doc(meta_store)
    # 연결 없는 두 엔티티: step 은 KakaoPay 만 표면 매칭, AuthService 는
    # 오직 query embedding 보강으로만 도달 가능.
    await graph_store.save_graph_data(doc_id, GraphData(
        entities=[
            Entity(name="AuthService", entity_type="service"),
            Entity(name="KakaoPay", entity_type="team"),
    ...
    # 보강 on (기본): AuthService 가 보강으로 유입
    boosted = await execute_graph_search(
        plan, graph_store, query_embedding=[1.0, 0.0],
    )
    assert boosted is not None
    assert "AuthService" in {e.name for e in boosted.entities}

    # 보강 off: KakaoPay 만 남는다
    unboosted = await execute_graph_search(
        plan, graph_store,
        query_embedding=[1.0, 0.0],
        enable_query_boost=False,
    )
    assert unboosted is not None
    names = {e.name for e in unboosted.entities}
    assert "KakaoPay" in names
    assert "AuthService" not in names


@pytest.mark.asyncio
async def test_execute_search_seed_fallback_disabled_skips_seed_embedding(
    graph_store: GraphStore, meta_store: MetadataStore,
) -> None:
    """ablation no-seed-fallback: enable_seed_fallback=False 면 시드 이름
    임베딩이 아예 호출되지 않고 표면 매칭에만 의존한다."""
    doc_id = await _create_doc(meta_store)
    await graph_store.save_graph_data(doc_id, GraphData(
        entities=[Entity(name="AuthService", entity_type="system")],
        relations=[],
    ))
    build_embed = AsyncMock()
    build_embed.aembed_documents = AsyncMock(return_value=[[1.0, 0.0]])
    await graph_store.build_entity_embeddings(build_embed)

    seed_embed = AsyncMock()
    seed_embed.aembed_documents = AsyncMock(return_value=[[1.0, 0.0]])

    plan = GraphSearchPlan(
        should_search=True,
        search_steps=[SearchStep(entity_name="존재하지않는이름", depth=1)],
    )
    result = await execute_graph_search(
        plan, graph_store,
        embedding_client=seed_embed,
        enable_seed_fallback=False,
    )
    # 표면 매칭 실패 + fallback 없음 + query_embedding 없음 → 결과 없음
    assert result is None
    # 시드 이름 임베딩 호출 자체가 생략됐다
    seed_embed.aembed_documents.assert_not_called()
