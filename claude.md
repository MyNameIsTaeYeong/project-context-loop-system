...
**목표:** `confluence_mcp` / `git_code` 소스의 인덱싱이 **현재 어떻게 동작하는가**를 6단계(수집→전처리→청킹→임베딩→그래프추출→저장)로 분해하여 소스타입별 서술형 분석을 생성한다. `indexing-improvement`가 개선·구현 지향이라면, 본 하네스는 **동작 이해·문서화**에 초점(코드 수정 없음).

**트리거:** "인덱싱 과정/단계별 분석", "인덱싱이 어떻게 이루어지는지", "파이프라인 동작 파악", "수집/청킹/임베딩/그래프/저장 단계 분석", "특정 소스/단계만 분석", "다시/추가 분석" 요청 시 `indexing-analysis` 스킬을 사용하라. 개선·수정·구현 요청은 `indexing-improvement`로.

**변경 이력:**
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-05-22 | 초기 구성 (2 풀파이프라인 분석가 + indexing-analysis 오케스트레이터) | 전체 | 6단계 인덱싱 동작을 소스타입별로 서술 분석하는 요청 — 기존 개선 지향 분석가(청킹/그래프 2단계)와 분리하여 분석 전용 자산화 |
